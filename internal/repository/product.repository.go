...
import (
	...
	entity "github.com/hugolesta/go-inventory-api/internal/entity"
)

const (
	qryInsertProduct = `INSERT INTO PRODUCTS (name, description, price, created_by) VALUES (?, ?, ?, ?);`
	qryGetAllProducts  = `SELECT id,name,description,price,created_by FROM PRODUCTS`
	qryGetProductByID  = `SELECT id,name,description,price,created_by FROM PRODUCTS WHERE id = ?`
)
